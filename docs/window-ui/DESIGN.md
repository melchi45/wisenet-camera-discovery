...
| 1.70 | 2026-09-10 | Youngho Kim | SRS.md FR-7.6 v2.48, reported directly by the user: `onSelectedTimeChange` (editing Selected Time's five fields, including toggling `#selected_has_end_time`, while PLAYING) wrote `player.startTime`/`endTime` but never reached the device — those setters (`@melchi45/rtsp-over-websocket`'s `RTSPOverWebSocket.ts`) only update internal fields, and `generateRTSPURL()` is exclusively triggered from the `seekingTime` setter's `seeking()`, the same mechanism `onDoubleClick`/`onCustomTimeSeek` already use. Fixed by also setting `player.seekingTime = player.currentTimestamp` (current position, not the possibly-just-edited Start Time, so playback doesn't jump back to the start) whenever `readyState === PLAYING`. `generateRTSPURL()` reads `this.endTime` directly for `strEnd`/`end=` in both the camera and NVR branches regardless of `seekingTime`, so the new end time reaches the regenerated URL even though the seek target is the unchanged current position. |
| 1.71 | 2026-09-10 | Youngho Kim | SRS.md FR-4.6 v2.49, requested directly by the user (5 numbered steps): `#use_gmt` relabeled "GMT:" and mounted through `mountSwitch()` as a segmented Off\|On switch (`#use_gmt_toggle`), byte-for-byte the same `variant`/`options` shape as `#sunapi_toggle` — `window.html`'s `#gmt_info` block now mirrors `#sunapi_info`'s structure (outer `<label class="switch">`, inner toggle container wrapping the real `<input type="checkbox">`), and `setupDevice()` gained one more `mountSwitch()` call alongside the existing `#sunapi_toggle`/`#http_type_toggle` ones. Per `docs/switch-component/DESIGN.md`'s progressive-enhancement design, `mountSwitch()` never replaces or removes the original `<input id="use_gmt">` — every existing `.checked`/`.disabled` read/write in `device.ts` (`changeusegmt()`, `onchangetimezone()`, the SUNAPI dateInfo auto-detect branch) needed no changes. `setupDevice()` also now explicitly forces `#timezone`'s value to `'0'` (GMT) alongside its existing `disabled = true`, instead of implicitly defaulting to the first `<option>` (GMT-12:00) — `#use_gmt` was already unchecked by default from the markup, no code change needed there. Verified visually via a one-off Playwright screenshot against `npm run start`'s example server: `#use_gmt_toggle` renders identically to `#sunapi_toggle`, defaults to Off, and `#timezone` shows "GMT" disabled. While touching this area, noticed and left in place an already-uncommitted local fix to a related bug (not part of this change, predates it): `onchangetimezone()`'s `#use_gmt` sync used to force-*un*check on every `changetimezone` event — since `changeusegmt()` checking `#use_gmt` itself writes `player.GMT`, which synchronously re-fires `changetimezone`, checking the switch used to immediately undo itself. It now force-*checks* instead, which is idempotent with a user's own check and matches what the SUNAPI-connected auto-detect path (FR-4.6, unchanged) already sets on success. See SRS.md FR-4.6's rewritten body for the full current behavior. |
| 1.72 | 2026-09-10 | Youngho Kim | SRS.md FR-4.6 v2.50, reported directly by the user with an exact repro (GMT On -> Off -> On while SUNAPI On) and a console stack trace pointing at `changeusegmt()`'s own `player.GMT = ...` line. Root cause: `#timezone`'s `<select>` `.value` is always a `string`, but `changeusegmt()`/`changetimezone()` assigned it to `player.GMT` unconverted; `@melchi45/rtsp-over-websocket`'s `set GMT()` recently tightened its validation to reject any non-`number` outright (previously a string silently passed its `<`/`>` range check via coercion), turning this pre-existing type mismatch into a real, reproducible throw on every manual re-check of the switch. Fixed by wrapping both call sites' `.value` reads in `parseFloat()`. Verified with a Playwright script driving the exact On/Off/On sequence against `npm run start`'s example server — no `set GMT` error, `player.GMT` ends up `0` as expected. The identical bug exists in `src/shared/window.ts`'s equivalent functions (`changeusegmt()`/`changetimezone()`, lines ~2470/2482) — left unfixed, out of scope for this `src/shared-v2/`-only report. |
| 1.73 | 2026-09-10 | Youngho Kim | SRS.md FR-7.1/FR-7.8 v2.51, reported directly by the user: turning SUNAPI Off while Play Type was already Playback re-connected SUNAPI and re-checked `#use_sunapi_client_checkbox` on its own. Traced to `updatePlaybackSunapiUIVisibility()` (`playbackCalendar.ts`): `showManual` (`isPlayback && !showCalendar`) went `true` the instant SUNAPI turned Off (since `showCalendar` requires `isSunapiOn`), which fired the manual flow's own default-search auto-fire (`updateManualPlaybackPanelVisibility(true)` -> `runManualTimelineSearch()`), whose documented "self-initializing a SUNAPI session if needed" convenience (FR-7.1) re-called `initSunapiManager()` because `on_change_use_sunapi_client()`'s Off branch had just cleared `player.sunapiClient` — and that call's success handler re-checks the switch. Presented the user two fix directions (suppress the auto-reconnect only for an explicit Off vs. require SUNAPI On for any Playback search UI); the user chose the latter. `showManual` now also requires `isSunapiOn`, and `#playback-calendar-timeline` (the `#timeline` wrapper) now requires `isPlayback && isSunapiOn` instead of `isPlayback` alone — SUNAPI Off hides all Playback search UI (both sub-panels and the timeline) regardless of Play Type, and FR-7.1's manual-flow self-init convenience is effectively retired (its panel is never shown without SUNAPI already On any more). Verified with a Playwright script: set `player.sunapiClient` to a dummy object + check the switch directly (no real device needed), switch Play Type to Playback (Calendar panel shows, as before), then flip SUNAPI Off — confirmed the switch stays Off, both search panels and the timeline wrapper hide, and no SUNAPI-ish request fires afterward. |
| 1.74 | 2026-09-10 | Youngho Kim | SRS.md FR-8.6 v2.52, requested directly by the user right after the same fix landed in `src/shared/window.ts` — `#talk` restyled "Talk:" + `mountSwitch()` segmented Off\|On (`#talk_toggle`) and wired via new `changetalk()` (`talk(flag)` is a real player method, not a gettable/settable property, gated on `isplay` same as `mute()`/`unmute()`); `#unmute`/`#mute` restyled as a button-group switch (`#mute_toggle`, `data-value="unmuted"`/`"muted"`), existing click listeners untouched, `onchangemute()` now also syncs a new `muteSwitch: SwitchController` via `setValue()`. `#talk` moves out of SRS.md's "Known dead controls" table — not a `src/shared/`-vs-`src/shared-v2/` divergence this time (unlike `#forward`/`#backward`'s v1.42 entry above), since both trees were fixed in the same session and now wire `#talk` identically; no new "Deviations from legacy behavior" bullet needed. Verified with the same Playwright approach as the `src/shared/` fix (`player.talk` stubbed, `isplay` overridden via `Object.defineProperty` since it's getter-only) — `changetalk()` correctly calls `talk(true)`/`talk(false)` on toggle — and a screenshot confirming `#talk_toggle`/`#mute_toggle` render identically to `src/shared/`'s own. See `docs/switch-component/`'s FR-12 v1.2/DESIGN.md v1.1 (the disabled-checkbox/button-group CSS this also needed) and `MEMORY.md`. |
| 1.75 | 2026-09-10 | Youngho Kim | SRS.md FR-8.1/FR-8.4/FR-8.5/FR-8.6 v2.53, reported directly by the user with a real device RTSP log showing a full TEARDOWN+reconnect (fresh `Session`, `CSeq` reset to 1) attributed to clicking Mute/Unmute. Traced `@melchi45/rtsp-over-websocket`'s call chains end to end: `mute()`/`unmute()` (`'audioIn'`) → `StreamPlayer.ts`'s `controlAudioIn()` → `MediaRouter.ts`'s `sendCommandData('audioIn', ...)` → `controlAudioPlayer()` — a local `mute` flag plus, for `type === 'canvas'` players, `createAudioPlayer()`/`deleteAudioPlayer()` (local decoder lifecycle) — no `RtspClient` call anywhere in this path. `talk()` (`'audioOut'`) → `controlAudioOut()` → `open(null, audioOutStatus)`, whose `info === null` branch (`StreamPlayer.ts` ~L433-440) unconditionally `close()`s then, once Teardown/Options is reached, `startStreaming()`s — the exact TEARDOWN→OPTIONS→DESCRIBE→SETUP×3→PLAY shape in the pasted log. So the reconnect was real, just from Talk, not Mute/Unmute — `#talk`'s `disabled` state had been tied to `onchangemute()`'s mute/unmute status (an artifact of the v2.52 restyle reusing the pre-existing coupling verbatim), which the user then gave the *correct* spec for instead: `#unmute`/`#mute` start disabled, default Muted; enabled (still Muted) on connect+Play; Unmute plays local RTP audio, no reconnect; `#talk` starts disabled, enabled only for Live-mode `PLAYING`, disabled for Playback (where its own reconnect is the expected, correct behavior for starting 2-way audio). Implemented: `videoControl.ts`'s `onstatechange()` `PLAYING` case now sets `#talk.disabled = playType !== LIVE` directly (own line, independent of the existing `el.ismute`-gated `#unmute` enable right above it); `audio.ts`'s `onchangemute()` no longer touches `#talk`; `setupAudio()` calls `muteSwitch.setValue('muted')` right after mounting `#mute_toggle`, matching `MediaRouter.ts`'s `_mute = true` default (`mountButtonGroup()`'s own default-active-first-option logic doesn't know about that field, so without this it showed "Unmute" active on a switch that starts logically muted). Verified with Playwright — first attempt showed "Unmute" still active post-fix, traced to `waitUntil: 'load'` resolving before `setupAudio()`'s module script had actually run; `waitUntil: 'networkidle'` (+ a short wait) confirmed "Mute" correctly active. `src/shared/window.ts` deliberately untouched — the user asked mid-session not to modify that tree further. |
