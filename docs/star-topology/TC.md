...
| TC-3 | Group by IP Address | ≥2 devices across ≥2 `/24` subnets | Leave `Group by` on `IP Address` | One hub per distinct `/24`, labeled `"{subnet}.0/24"`; each device is a leaf under the hub matching its own subnet (FR-4/FR-5). |
...
| TC-7 | Hub nodes are never linked to each other | Any grouping with ≥2 hubs | Visually inspect the graph | No edge connects any hub node to another hub node — only hub→leaf edges exist (FR-7). |
