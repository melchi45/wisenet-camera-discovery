...
| 1.1 | 2026-09-10 | Youngho Kim | SRS.md FR-12 v1.2: two new callers in `src/shared/window.ts`, requested directly by the user against `window.html`'s `#talk`/`#unmute`/`#mute`. `#talk_toggle` (checkbox target) is the first real caller of a *disabled* checkbox switch — added `.ws-switch--segmented input[type="checkbox"]:disabled ~ .ws-switch-option` (grays the non-current option, same `--button-disable-color`/`--button-disable-font-color` as the existing radio rule) since a `display:none` disabled checkbox has no visual path of its own. `#mute_toggle` (button-group target, `#unmute`/`#mute`) is the first real caller of a disabled *button* switch — added `.ws-switch--segmented button.ws-switch-option:disabled`/`:disabled:not(.active)`, needed because a `<button>` is itself the visible element (unlike the hidden radio/checkbox inputs), so its custom `background`/`color` isn't reset by any native browser disabled treatment on its own. Both rules keep the option reflecting the *current* state fully accented even while disabled, matching the existing radio rule's own behavior. Neither `window.html` restructuring (`#talk` wrapped in `#talk_toggle`; `#unmute`/`#mute` wrapped in `#mute_toggle` with `data-value="unmuted"`/`"muted"`) nor the two `mountSwitch()` calls needed any change to `switch.ts` itself — CSS-only, matching this component's own progressive-enhancement, non-destructive design. Also fixed a real, separate bug found while wiring this: `#talk` had never actually been connected to anything — checking/unchecking it had no effect. `changetalk()` (new) now calls the player's `talk(flag: boolean)` method (an `'audioOut'` control command in `@melchi45/rtsp-over-websocket`'s `RTSPOverWebSocket.ts`), gated on `isplay` the same way the pre-existing `mute()`/`unmute()` already are. A new `muteSwitch: SwitchController` is also synced from `onchangemute()` (the player's own `'changemute'` event handler) via `setValue()`, so a device-driven mute change (not just a click on either button) is reflected in the switch's visual active state too. Verified with a Playwright script (`player.talk` stubbed, `isplay` overridden via `Object.defineProperty` since it's a getter-only accessor with no public setter) confirming `changetalk()` calls `talk(true)`/`talk(false)` on toggle. `src/shared-v2/`'s `audio.ts` has the identical `#talk`/un-wired gap — left unfixed, out of scope (this request was specifically against `src/shared/`). |
