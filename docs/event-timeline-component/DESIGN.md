...
- No keyboard navigation *within* the timeline track itself (arrow-key pan, +/- zoom) — the toolbar's
  preset/zoom/reset/Hide buttons are real `<button>` elements and so are already keyboard-operable,
  but there is no keyboard equivalent for wheel-zoom or drag-pan on the track/overview row.
